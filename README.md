...
**CSV Import**
- Upload and import up to 500,000 product records
...
2. **Create an environment file** (optional — defaults are set)
   ```bash
...
4. Monitor the upload progress in real time:
   ...
   - Status messages indicate the current stage (Parsing → Validating → Importing → Complete)
   - Errors are displayed if any occur

### Product Management

1. Navigate to the **Products** tab.
2. Use filters to search by SKU, name, status, or description.
3. **Create Product** — click "+ Add Product"
4. **Edit Product** — click "Edit" on any product row
5. **Delete Product** — click "Delete" (with confirmation)
6. **Bulk Delete** — click "🗑️ Delete All" (requires confirmation)
...
3. **Test Webhook** — click "Test" to send a test event and view response status code and response time
4. **Edit / Delete** — use the respective buttons on each webhook card
...
All services are configured via environment variables (defaults set in `docker-compose.yml`):
...
4. Add environment variables: `DATABASE_URL`, `REDIS_URL`, `CELERY_BROKER_URL`, `CELERY_RESULT_BACKEND`
5. Create a separate **Background Worker**:
...
3. **Webhook testing**
   ...
   - Configure a webhook and trigger a test event
...
| Issue | Checks |
|---|---|
| Database connection errors | Ensure PostgreSQL is running · check `DATABASE_URL` · verify the database exists |
| Celery tasks not executing | Check Redis is running · verify the Celery worker is started · check `CELERY_BROKER_URL` and `CELERY_RESULT_BACKEND` |
| Upload progress not updating | Check the WebSocket connection in the browser console · verify the Celery task is running · check the Redis connection |
| Memory issues with large files | Ensure chunked processing is working · check available system memory · reduce `CHUNK_SIZE` in config if needed |
